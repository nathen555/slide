...
function getLocalSaves() { try { const saved = JSON.parse(localStorage.getItem(SAVED_DECK_KEY)); return saved?.slides?.length ? [saved] : []; } catch { return []; } }
function saveIdentity(deck) { return `${deck.title}|${(deck.sources || [deck.title]).join('|')}`; }
function renderSavedSaves(saves) { state.savedSaves = saves; $('#saved-grid').innerHTML = saves.length ? saves.map((deck, index) => `<article class="saved-card"><div class="saved-thumb"><span>${escapeHtml(deck.slides[0]?.title || deck.title).slice(0, 42)}</span><b>${String(index + 1).padStart(2, '0')}</b></div><div class="saved-card-body"><h3>${escapeHtml(deck.title)}</h3><p>${deck.slides.length} slides · ${deck.sources?.length || 1} source${(deck.sources?.length || 1) === 1 ? '' : 's'}</p><div class="saved-card-actions"><button class="open-save" data-save-index="${index}" type="button">Open notes ↗</button><button class="delete-save" data-save-index="${index}" type="button">Delete</button></div></div></article>`).join('') : '<p class="empty-state">No saved notes yet. Mine a deck and choose Save notes.</p>'; }
async function loadSaves() { const saves = getLocalSaves(); if (supabaseClient) { const { data } = await supabaseClient.from('saved_notes').select('title,deck,updated_at').eq('browser_id', getBrowserId()).order('updated_at', { ascending: false }); if (data?.length) saves.push(...data.map((row) => row.deck)); } renderSavedSaves([...new Map(saves.map((deck) => [saveIdentity(deck), deck])).values()]); }
...
async function deleteSaved(index) { const deck = state.savedSaves[index]; if (!deck) return; const remaining = state.savedSaves.filter((_, saveIndex) => saveIndex !== index); localStorage.setItem(SAVED_DECK_KEY, remaining[0] ? JSON.stringify(remaining[0]) : ''); if (supabaseClient) await supabaseClient.from('saved_notes').delete().eq('browser_id', getBrowserId()).eq('title', deck.title); renderSavedSaves(remaining); }
$('#saved-grid').addEventListener('click', (event) => { const deleteButton = event.target.closest('.delete-save'); if (deleteButton) { pendingDeleteIndex = Number(deleteButton.dataset.saveIndex); const deck = state.savedSaves[pendingDeleteIndex]; if (!deck) return; $('#delete-dialog-copy').textContent = `“${deck.title}” will be removed from your saved notes. This cannot be undone.`; $('#delete-dialog').classList.remove('is-hidden'); $('#cancel-delete').focus(); return; } const button = event.target.closest('.open-save'); if (!button) return; const deck = state.savedSaves[Number(button.dataset.saveIndex)]; if (deck) { renderDeck(deck); document.querySelector('[data-section="top"]').click(); } });
$('#cancel-delete').addEventListener('click', () => { pendingDeleteIndex = null; $('#delete-dialog').classList.add('is-hidden'); });
$('#confirm-delete').addEventListener('click', async () => { const index = pendingDeleteIndex; pendingDeleteIndex = null; $('#delete-dialog').classList.add('is-hidden'); await deleteSaved(index); });
